/**
 * for Steam OpenID
 * https://github.com/SteamOff/steamoff/blob/master/app/utils/OpenID.scala
 * @author jege
 */
package utils
...
object UserInfo {
   ...
   */
  private[utils] class UserInfoExtractor(params: Map[String, Seq[String]]) {
...
private[utils] class OpenIDClient(ws: String => WSRequestHolder) {
  ...
  private def verifiedId(queryString: Map[String, Seq[String]]): Future[UserInfo] = {
    (queryString.get("openid.mode").flatMap(_.headOption),
      ...
      case (Some("id_res"), Some(id)) => {
        ...
        server.flatMap(directVerification(queryString))(global)
      }
      case (Some("cancel"), _) => throw Errors.AUTH_CANCEL
 ...
 */
private[utils] class Discovery(ws: (String) => WSRequestHolder) {
  import Discovery._
...
private[utils] object Discovery {
